...
public class InvenDAO {
    ...
    Connection conectar() throws SQLException, ClassNotFoundException{
     ...
     public ArrayList<Inventario> BuscarInventario(String opc, String texto){
      ...
      public boolean guardarfactura(Inven iInven) {
        try{
            ps = conectar().prepareStatement("INSERT INTO detallesfactura (id_pro, precio,cantidad,subtotal) VALUES (?,?,?,?)");
            ps.setInt(1, iInven.getIdinventario());
            ps.setDouble(2, iInven.getPrecio_unitario());
            ps.setInt(3, iInven.getCantidad());
            ps.setDouble(4, iInven.getSubtotal());

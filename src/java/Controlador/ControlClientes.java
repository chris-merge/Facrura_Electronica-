...
public class ControlClientes extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           if ("reporte".equals(request.getParameter("accion"))) {
                List<Clientes> listaClientes = obtenerClientes(); // Método para obtener la lista de clientes
                request.setAttribute("DatosClientes", listaClientes);
                request.getRequestDispatcher("ReporteClientes.jsp").forward(request, response);
            }
           if ("mostrar".equals(request.getParameter("accion"))) {
                List<Clientes> listaClientes = obtenerClientes(); // Método que obtiene los clientes
                request.getSession().setAttribute("DatosClientes", listaClientes);
                request.getRequestDispatcher("VistaPrincipal.jsp").forward(request, response);
            }

           rd = request.getRequestDispatcher("VistaClientes.jsp");
